...
class Obstacle(pygame.sprite.Sprite):
	def __init__(self,type):
		...
		if type == 'fly':
			...
			barnacle_1 = pygame.image.load('runner\graphics\\barnacle\\barnacle_bite.png').convert_alpha()
			barnacle_2 = pygame.image.load('runner\graphics\\barnacle\\barnacle.png').convert_alpha()
			self.frames = [barnacle_1,barnacle_2]
			y_pos = 300
		elif type == 'bat':
			bat_1 = pygame.image.load('runner\graphics\\bat\\bat.png').convert_alpha()
			bat_2 = pygame.image.load('runner\graphics\\bat\\bat_fly.png').convert_alpha()
			self.frames = [bat_1,bat_2]
			y_pos = 170
		elif type == 'spinner':
			spinner_1 = pygame.image.load('runner\graphics\spinner\spinnerHalf.png').convert_alpha()
			spinner_2 = pygame.image.load('runner\graphics\spinner\spinnerHalf_spin.png').convert_alpha()
			self.frames = [spinner_1,spinner_2]
			y_pos = 300
		else:
...
pixel_font = pygame.font.Font('runner\Pixeltype.ttf', 50)
game_active = False
start_time = 0
score = 0
	...
	for event in pygame.event.get():
		...
		if game_active:
			if event.type == obstacle_timer:
				obstacle_group.add(Obstacle(choice(['fly','snail','snail','barnacle', 'bat', 'spinner'])))
